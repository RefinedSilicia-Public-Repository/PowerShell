...
$win64RootFolder = 'C:\Program Files (x86)\LansweeperAgent'
$win32RootFolder = 'C:\Program Files\LansweeperAgent'
#Delcare variables necessary from endpoint
$ComputerNameString = $env:COMPUTERNAME
#$OSVersionString = (Get-CimInstance Win32_OperatingSystem).version
$OSVersionString = Get-WmiObject -class Win32_OperatingSystem | select Version
...
"6.*" {
    ...
    if((Test-Path -Path $win64RootFolder)){
        WriteLog "Script 6.* did not write to $ComputerNameString with 64 bit variable due to Lansweeper already installed."
    }
    elseif((Test-Path -Path $win32RootFolder)){
        WriteLog "Script 6.* did not write to $ComputerNameString with 32 bit variable due to Lansweeper already installed."
    }
    Else{
...
"10.*" {
    ...
    #Calls Sysvol on domain controller to deploy exe
    if((Test-Path -Path $win64RootFolder)){
        WriteLog "Script 10.* did not write to $ComputerNameString with 64 bit variable due to Lansweeper already installed."
    }
    elseif((Test-Path -Path $win32RootFolder)){
        WriteLog "Script 10.* did not write to $ComputerNameString with 32 bit variable due to Lansweeper already installed."
    }
    Else{
        Start-Process -FilePath "\\corp.sgstool.com\SYSVOL\corp.sgstool.com\exe\LsAgent-windows_12.2.0.1.exe" -ArgumentList ' --server Lansweeper.corp.sgstool.com --port 9524 --agentkey ece3aac3-fc25-4ba2-a318-a8ef69d6966e --mode unattended'
...
default{
   WriteLog "No Matches Found for $ComputerNameString skipping machine $ComputerNameString"
    }
}
